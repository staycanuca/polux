...
addon_data_dir = os.path.join(xbmc.translatePath("special://home/addons/" ).decode("utf-8"), AddonID)
...
def PlayUrl(name, url, iconimage=None):
	...
	if iconimage is not None:
		...
		url = 'plugin://program.plexus/?mode=2&url={0}&name={1}'.format(url, name, iconimage)
	xbmc.log('--- Playing "{0}". {1}'.format(name, url), 2)
	listitem = xbmcgui.ListItem(path=url)
	listitem.setInfo(type="Video", infoLabels={"mediatype": "movie", "Title": name })
	if iconimage is not None:
