...
addon = xbmcaddon.Addon('plugin.video.streams-fork')
...
def addon_log(string):
  ...
  if DEBUG == 'true':
    ...
    xbmc.log("[plugin.video.streams-fork-%s]: %s" %(ADDON_VERSION, string))
...
def extract(file_tar,destination):
    dp = xbmcgui.DialogProgress()
    dp.create("Streams-fork","Extracting module contents. Please wait.")
...
def remove(file_):
    dp = xbmcgui.DialogProgress()
    dp.create("Streams-fork","Removing files.")
    os.remove(file_)
    dp.update(100)
    dp.close()
...
def acekit(acestream_pack):
    ...
    Downloader(acestream_pack,ACE_KIT,"Downloading AceStream modules.","Streams-fork")
