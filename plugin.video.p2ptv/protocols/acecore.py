...
""" Copyright (c) 2017 Mario Bălănică
    ...
    This file contains the AceStream console of the addon.
    ...
    OverlayText(object) -> Overlay text displayed on player.
...
class TSengine():
        ...
        if xbmc.getCondVisibility('System.Platform.Android'):
            try:
                if settings.getSetting('engine_app') == "1": xbmc.executebuiltin('XBMC.StartAndroidActivity("org.acestream.engine")')
                    ...
                    command = ["sh","/data/data/"+settings.getSetting('app_id')+"/files/plugin.video.p2ptv/org.acestream.engine/files/droidace.sh",settings.getSetting('app_id')]
            ...
            if "arm" in os.uname()[4]:
                try:
                    command = ["sh",os.path.join(addonprofile,"acestream","start_acestream.sh"),"--client-console"]
                    if settings.getSetting('total_max_download_rate') != "0":
                        command.append('--download-limit')
                        command.append(settings.getSetting('total_max_download_rate'))
                    if settings.getSetting('total_max_upload_rate') != "0":
                ...
                try:
                    command = ["sh",os.path.join(addonprofile,'acestream','start.sh')]
                    if settings.getSetting('total_max_download_rate') != "0":
                        command.append('--download-limit')
                        command.append(settings.getSetting('total_max_download_rate'))
                    if settings.getSetting('total_max_upload_rate') != "0":
                ...
                if settings.getSetting('ace_cmd') == "0":
                    acefolder = os.path.join(addonprofile,'acestream')
                    acebin = os.path.join(addonprofile,'acestream','acestreamengine')
    ...
    def startWin(self):
        try:
            ...
            path_value=os.path.join(addonprofile,'acestream',needed_value)
    ...
    def getWinPort(self):
        try:
            path=os.path.join(addonprofile,'acestream')
            pfile= os.path.join( path,'acestream.port')
            gf = open(pfile, 'r')
            aceport=int(gf.read())
        except: 
...
def stop_aceengine():
    if settings.getSetting('shutdown-engine') == 'true' and settings.getSetting('kill_type') == '0':
        if xbmc.getCondVisibility('system.platform.windows'):
            ...
            if "arm" in os.uname()[4]:
                os.system("sh "+os.path.join(addonprofile,"acestream","stop_acestream.sh"))
            else:
