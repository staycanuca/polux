...
base_url = "https://arenavision.biz/"
	...
	if source:
		...
		for link, nome in match:
			if "events" in nome.lower():
				addDir("[B][COLOR red]Agenda/Schedule[/COLOR][/B]",base_url+link,401,os.path.join(current_dir,"icon.png"),1,True,parser="arenavision-biz",parserfunction="arenavision_schedule")
			elif ("#" in nome.lower() or "av" in nome.lower() or "ArenaVision" in nome.lower()) and link != "/":
				addDir(nome,base_url+link,401,os.path.join(current_dir,"icon.png"),1,False,parser="arenavision-biz",parserfunction="arenavision_streams")
			else: pass
...
def arenavision_schedule(url):
	...
	if source:
		...
		for day, month, year, hour, minute, sport, competition, event, channelsStr in events:
			...
			addDir('[B][COLOR red]{0}[/COLOR] [COLOR white]{1}[/COLOR][/B] {3} [COLOR yellow][{2}][/COLOR]'.format(timeStr, removeNonAscii(sport), removeNonAscii(competition), removeNonAscii(event)), str(streams),401,os.path.join(current_dir,"icon.png"),1,False,parser="arenavision-biz",parserfunction="arenavision_chooser")
		return None
...
def arenavision_chooser(url):
	...
	if index > -1:
		arenavision_streams("ArenaVision {0}".format(channelList[index]), "https://arenavision.biz" + "/" + str(channelList[index]).zfill(2))
