...
def index():
	...
	addDir ('Community Builds'        ,'builds',   icon=ICONBUILDS,   themeit=THEME1)
	addDir ('Maintenance Tools'   ,'maint',    icon=ICONMAINT,    themeit=THEME1)
	if wiz.platform() == 'android' or DEVELOPER == 'true': addDir ('Apk Installer' ,'apk', icon=ICONAPK, themeit=THEME1)
	if not ADDONFILE == 'http://': addDir ('Addon And Repo Installer' ,'addons', icon=ICONADDONS, themeit=THEME1)
	if not YOUTUBEFILE == 'http://' and not YOUTUBETITLE == '': addDir (YOUTUBETITLE ,'youtube', icon=ICONYOUTUBE, themeit=THEME1)
	addDir ('Save Debrid And Trakt Data'     ,'savedata', icon=ICONSAVE,     themeit=THEME1)
	addDir ('GitHub / Zip Installer'     ,'call_installer', icon=ICONSAVE,     themeit=THEME1)
	if HIDECONTACT == 'No': addFile('[COLOR white]Contact[/COLOR]' ,'contact', icon=ICONCONTACT,  themeit=THEME1)
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
...
def maintMenu(view=None):
	...
	if includevid == 'true':
		...
		addFile('--- Include 1Channel: %s' % include1ch.replace('true',on).replace('false',off), 'togglecache', 'include1channel', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include Alluc: %s' % includeall.replace('true',on).replace('false',off), 'togglecache', 'includealluc', icon=ICONMAINT, themeit=THEME3)
		...
		addFile('--- Include BOB Unleashed: %s' % includebob.replace('true',on).replace('false',off), 'togglecache', 'includebobunleashed', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include Bubbles: %s' % includebub.replace('true',on).replace('false',off), 'togglecache', 'includebubbles', icon=ICONMAINT, themeit=THEME3)
		...
		addFile('--- Include Gurzil: %s' % includegur.replace('true',on).replace('false',off), 'togglecache', 'includegurzil', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include Icefilms: %s' % includeice.replace('true',on).replace('false',off), 'togglecache', 'includeicefilms', icon=ICONMAINT, themeit=THEME3)
		...
		addFile('--- Include Tinklepad: %s' % includetin.replace('true',on).replace('false',off), 'togglecache', 'includetinklepad', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include UGOTTOC: %s' % includeugo.replace('true',on).replace('false',off), 'togglecache', 'includeugottoc', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include XXXODUS: %s' % includexxx.replace('true',on).replace('false',off), 'togglecache', 'includexxxodus', icon=ICONMAINT, themeit=THEME3)
		...
		addFile('--- Include ALLUC: %s' % includeall.replace('true',on).replace('false',off), 'togglecache', 'includealluc', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include ONECHANNEL: %s' % includeone.replace('true',on).replace('false',off), 'togglecache', 'includeonechannel', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include UGOTTOC: %s' % includeugo.replace('true',on).replace('false',off), 'togglecache', 'includeugottoc', icon=ICONMAINT, themeit=THEME3)		
