...
INCLUDE1CHANNEL   = ADDON.getSetting('include1channel')
INCLUDEALLUC   = ADDON.getSetting('includealluc')
INCLUDEBENNU   = ADDON.getSetting('includebennu')
INCLUDEBOBUNLEASHED   = ADDON.getSetting('includebobunleashed')
INCLUDEBUBBLES   = ADDON.getSetting('includebubbles')
INCLUDECOVENANT   = ADDON.getSetting('includecovenant')
INCLUDEELYSIUM   = ADDON.getSetting('includeelysium')
INCLUDEXODUS   = ADDON.getSetting('includeexodus')
INCLUDEGURZIL   = ADDON.getSetting('includegurzil')
INCLUDEICEFILMS   = ADDON.getSetting('includeicefilms')
INCLUDESPECTO   = ADDON.getSetting('includespecto')
INCLUDETINKLEPAD   = ADDON.getSetting('includetinklepad')
INCLUDEUGOTTOC   = ADDON.getSetting('includeugottoc')
INCLUDEXXXODUS   = ADDON.getSetting('includexxxodus')
INCLUDESALTS   = ADDON.getSetting('includesalts')
INCLUDEALLUC   = ADDON.getSetting('includealluc')
INCLUDEONECHANNEL   = ADDON.getSetting('includeonechannel')
INCLUDEUGOTTOC   = ADDON.getSetting('includeugottoc')
...
def getCacheSize():
	...
	dbfiles   = [
		(os.path.join(ADDONDATA, 'plugin.video.1channel', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.alluc.api', 'alluc.api.db')),
		(os.path.join(ADDONDATA, 'plugin.video.bennu', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.bob.unleashed', 'database.db')),
		(os.path.join(ADDONDATA, 'plugin.video.bubbles', 'cache2.db')),
		...
		(os.path.join(ADDONDATA, 'plugin.video.icefilms', 'ice_cache.db')),			
		(os.path.join(ADDONDATA, 'plugin.video.specto', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.tinklepad', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.ugottoc', 'alluc.api.db')),
		(os.path.join(ADDONDATA, 'plugin.video.xxx-o-dus', 'cache.db')),
		(os.path.join(DATABASE,  'alluc.db')),
		(os.path.join(DATABASE,  'saltscache.db')),
		(os.path.join(DATABASE,  'onechannelcache.db')),
		(os.path.join(DATABASE,  'ugottoc.db'))]
	cachelist = [
		(PROFILEADDONDATA),
		(ADDONDATA),
	...
	if INCLUDEVIDEO == 'true':
		...
		if INCLUDEALL == 'true': files = dbfiles
			...
			if INCLUDE1CHANNEL == 'true':     files.append(os.path.join(ADDONDATA, 'plugin.video.1channel', 'cache.db'))
			if INCLUDEALLUC == 'true':     files.append(os.path.join(ADDONDATA, 'plugin.video.alluc.api', 'alluc.api.db'))			
			if INCLUDEBENNU == 'true':     files.append(os.path.join(ADDONDATA, 'plugin.video.bennu', 'cache.db'))
			if INCLUDEBOBUNLEASHED == 'true':     files.append(os.path.join(ADDONDATA, 'plugin.video.bob.unleashed', 'database.db'))
			if INCLUDEBUBBLES == 'true':     files.append(os.path.join(ADDONDATA, 'plugin.video.bubbles', 'cache2.db'))
			...
			if INCLUDEXODUS == 'true':     files.append(os.path.join(ADDONDATA, 'plugin.video.exodus', 'cache.db'))
			if INCLUDEGURZIL == 'true':     files.append(os.path.join(ADDONDATA, 'plugin.video.gurzil', 'cache.db'))
			if INCLUDEICEFILMS == 'true':     files.append(os.path.join(ADDONDATA, 'plugin.video.icefilms', 'ice_cache.db'))
			if INCLUDESPECTO == 'true':     files.append(os.path.join(ADDONDATA, 'plugin.video.specto', 'cache.db'))
			if INCLUDETINKLEPAD == 'true':     files.append(os.path.join(ADDONDATA, 'plugin.video.tinklepad', 'cache.db'))
			if INCLUDEUGOTTOC == 'true':     files.append(os.path.join(ADDONDATA, 'plugin.video.ugottoc', 'alluc.api.db'))
			if INCLUDEXXXODUS == 'true':     files.append(os.path.join(ADDONDATA, 'plugin.video.xxx-o-dus', 'cache.db'))
			if INCLUDESALTS == 'true':   files.append(os.path.join(DATABASE,  'saltscache.db'))
			if INCLUDEALLUC == 'true':   files.append(os.path.join(DATABASE,  'alluc.db'))		
			if INCLUDEONECHANNEL == 'true':   files.append(os.path.join(DATABASE,  'onechannelcache.db'))
			if INCLUDEUGOTTOC == 'true': files.append(os.path.join(DATABASE,  'ugottoc.db'))
		if len(files) > 0:
